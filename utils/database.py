...
class Database:
    ...
    # --- PHẦN LƯU TRỮ BẢNG LƯƠNG TỔNG HỢP (NEW) ---
    ...
    def get_payroll_status(self, year, month):
        """Kiểm tra xem tháng này đã có dữ liệu Approved chưa"""
    ...
    def save_payroll_data(self, df_to_save, year, month, status="Draft", creator="System"):
        """Ghi Snapshot 32+ cột bảng lương vào Sheets"""
        sh = self._open_pay_file(year)
        ...
        try:
            ...
            for row in final_df.values.tolist():
                final_list.append([self._clean_for_sheets(v) for v in row])
                
            ws.clear()
            ws.update(final_list, value_input_option='USER_ENTERED')
            return True
        except Exception as e:
            st.error(f"Lỗi khi lưu bảng lương: {e}"); return False
