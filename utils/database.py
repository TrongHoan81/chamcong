...
class Database:
    ...
    def update_employee(self, employee_id, updated_data, move_log=None):
        """Cập nhật hồ sơ và ghi lịch sử (Nâng cấp ghi nhận Chức danh đi/đến)"""
        try:
            ...
            if move_log:
                ...
                # Thứ tự cột chuẩn: ID, Tên, Loại, Đơn vị đi, Đơn vị đến, Chức danh đi, Chức danh đến, Ngày
                ws_log.append_row([
                    ...
                    self._clean_for_sheets(move_log.get('from_pos', '-')), # Mới
                    self._clean_for_sheets(move_log.get('to_pos', '-')),   # Mới
