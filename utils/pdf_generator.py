...
def export_attendance_pdf(df, unit_name, month, year, status, shift_type="Normal"):
    ...
    cw = {'stt': 7, 'name': 40, 'pos': 15, 'day': 5.5, 'summary': 8.9}
    ...
    for i in range(1, 32): pdf.cell(cw['day'], h_row2, str(i), 1, 0, 'C', True)
    ...
    for i, row in df.iterrows():
        ...
        for col in ["Công sản phẩm", "Công thời gian", "Ngừng việc 100%", "Ngừng việc < 100%", "Hưởng BHXH"]:
            pdf.cell(cw['summary'], 7, str(row.get(col, 0)), 1, 0 if col != "Hưởng BHXH" else 1, 'C')
